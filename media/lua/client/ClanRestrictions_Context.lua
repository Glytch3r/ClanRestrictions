...
function ClanRestrictions.context(player, context, worldobjects, test)
    ...
    if not SandboxVars.ClanRestrictions.AnySquareAccess then
        local sqSh = SafeHouse.getSafeHouse(sq)
        if not sqSh or sqSh ~= playerSh then return end
    end
    ...
    infoSub:addOption(string.format("Coordinates: (X1:%d, Y1:%d) (X2:%d, Y2:%d)", round(playerSh:getX()), round(playerSh:getY()), round(playerSh:getX2()), round(playerSh:getY2())), worldobjects, nil)
